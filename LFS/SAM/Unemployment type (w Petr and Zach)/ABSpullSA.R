## Purpose: The set of calibration targets for a SAM model, using pre-COVID data
# Using externally SA data where possible.
# Update 24 June 2024 to include "levels" as well as standard deviations
# Author: Matt Nolan
...
### Setting up flows data ----
# It appears the starting month changes through time as the flows data is updated.
...
setDT(EmpTrans)
# Adjust EmpTrans to start at the first month of a quarter.

adj <- ifelse(month(EmpTrans$date[1]) %% 3 == 0,1,ifelse(month(EmpTrans$date[1]) %% 3 == 1,0,2))

EmpTrans <- EmpTrans[(1+adj):nrow(EmpTrans),]
...
# Adjust UnTrans to start at the first month of a quarter.

UnTrans <- UnTrans[(1+adj):nrow(UnTrans),]
...
Employed <- Employed[(1+adj):nrow(Employed),]
...
ggplot(vu,aes(x=date,y=vu)) + geom_line()+ geom_vline(xintercept = as.Date("2020-03-01"),colour="black",linetype = "dashed") + geom_vline(xintercept = as.Date("2008-09-01"),colour="black",linetype = "dashed") + geom_vline(xintercept = as.Date("1987-09-01"),colour="black",linetype = "dashed") + geom_vline(xintercept = as.Date("1997-06-01"),colour="black",linetype = "dashed") + geom_vline(xintercept = as.Date("1990-03-01"),colour="black",linetype = "dashed") + labs_e61(
  title = "Tightness measures",
  subtitle = "Vacancies per unemployed person",
  y="",
  sources = "ABS National Accounts.
Source: ABS, e61.",
)
...
JFRS <- tibble(date = JFR$date, JFRS = seas(ts(JFR$JFRQ,start=c(2005,3),end=c(2024,1),freq=4))$series$s11) %>% filter(date <= dateendtrend)  %>% mutate(lJFRS = log(JFRS)) %>% drop_na %>% mutate(lJFRS_HP = hpfilter(lJFRS,freq=hpf,type="lambda")$cycle)

SRS <- tibble(date = SR$date, SRS = seas(ts(SR$SRQ,start=c(2005,3),end=c(2024,1),freq=4))$series$s11)%>% filter(date <= dateendtrend)  %>% mutate(lSRS = log(SRS)) %>% drop_na %>% mutate(lSRS_HP = hpfilter(lSRS,freq=hpf,type="lambda")$cycle)

UES <- tibble(date = UE$date,UES = seas(ts(UE$UEQ,start=c(2005,3),end=c(2024,1),freq=4))$series$s11)%>% filter(date <= dateendtrend)  %>% mutate(lUES = log(UES)) %>% drop_na %>% mutate(lUES_HP = hpfilter(lUES,freq=hpf,type="lambda",drift=TRUE)$cycle)

EUS <- tibble(date = EU$date,EUS = seas(ts(EU$EUQ,start=c(2005,3),end=c(2024,1),freq=4))$series$s11)%>% filter(date <= dateendtrend)  %>% mutate(lEUS = log(EUS)) %>% drop_na %>% mutate(lEUS_HP = hpfilter(lEUS,freq=hpf,type="lambda",drift=TRUE)$cycle)
...
col_keep <- c("date",grep("_HP$", colnames(dfS), value = TRUE))
col_log <- c("date",grep("^l", colnames(dfS), value = TRUE))
col_log <- col_log[!grepl("_HP$", col_log)]
col_level <- colnames(dfS)
col_level <- col_level[!grepl("_HP$", col_level)]
col_level <- col_level[!grepl("^l",col_level)]
dfS_14_19_level <- dfS[,..col_level][date >= datestart & date <= dateend]
dfS_14_19_log <- dfS[,..col_log][date >= datestart & date <= dateend]

col_keep <- c("date",grep("_HP$", colnames(dfS), value = TRUE))
dfS <- dfS[,..col_keep]

dfS_14_19 <- dfS[date >= datestart & date <= dateend]

grep("_HP$", colnames(dfS), value = TRUE)
...
meanS <- sapply(dfS_14_19,mean,na.rm=TRUE)
stdS <- sapply(dfS_14_19,sd,na.rm=TRUE)
mean_level <- sapply(dfS_14_19_level,mean,na.rm=TRUE)
mean_log<- sapply(dfS_14_19_log,mean,na.rm=TRUE)

#correlationS <- cor(dfS_14_19[,2:ncol(dfS_14_19)],method="pearson") 
correlationS <- cor(dfS_14_19[,!("date"),with=FALSE],method="pearson") # Create version explicitly dropping the date incase column order shifted.
c <- ccf(dfS_14_19[,2],dfS_14_19[,9]) # UR and LP cyclical components CCF

elasticitytoPS <- data.frame(cov(dfS_14_19[,!("date"),with=FALSE])*100/stdS[ncol(dfS_14_19)]) %>% select(tail(colnames(dfS_14_19),1)) # Believe the elasticty figures are scaled up by 100 - but need to check this
...
for (i in 1:8){
  acS1[i] <- cor(a[2:nrow(a),..i],b[2:nrow(a),..i])[1] # Remove the NAs generated by the lags
}
...
meanlevel <- data.frame(variable = names(mean_level),mean_level)%>% filter(!variable %in%  c("date","LPS","vS","EUS","UES")) 
colnames(meanlevel) = c("variable","Mean_level")
meanlog <- data.frame(variable = names(mean_log),mean_log) %>% filter(!variable %in% c("date","lLPS","lvS","lEUS","lUES")) 
colnames(meanlog) = c("variable","Mean_log")
meanSF <- data.frame(variable = names(meanS),meanS) %>% filter(variable != "date") 
colnames(meanSF) = c("variable","Mean_filter")
...
datadictionary <- data.frame(names(acS1),c("Labour Productivity","Vacancies","Vacancy to Unemployment Ratio","U to E Transition Flow","E to U Transition Flow","Job Finding Rate","Separation Rate","Unemployment Rate"))
colnames(datadictionary) <- c("variable","Description")

exportdf <- list(meanlevel,meanlog,meanSF,stdSF,corrF,elasticitytoPSF,acS1F,datadictionary)
names(exportdf) <- c("Means_level","Means_log","Means_filter","Standard Deviations","Correlations with Productivity","Elasticities","Autocorrelations","Data Dictionary")

exportdf

library(openxlsx)

#write.xlsx(exportdf,"Targets_pre_COVID.xlsx") # The last file exported
write.xlsx(exportdf,"Targets_inc_COVID.xlsx") # The last file exported
