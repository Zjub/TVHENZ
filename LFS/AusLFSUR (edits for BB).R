...
# The time looks broken on a number of the early graphs - redo this from scratch for the report.
# Go to line 247 to update
...
ggplot(savesdf2 %>% filter(dates > "2017-12-01"),aes(x=dates,y=value,colour=variable)) + geom_line() + geom_vline(xintercept = as.Date("2019-12-01"),colour="red") + geom_vline(xintercept = as.Date("2020-09-01"),colour="blue") + geom_vline(xintercept = as.Date("2021-03-01"),colour="blue") + labs(title = "Employed to Unemployed")
...
ggplot(savesdf7 %>% filter(dates > "2017-12-01"),aes(x=dates,y=value,colour=variable)) + geom_line() + geom_vline(xintercept = as.Date("2020-02-01"),colour="green") + geom_vline(xintercept = as.Date("2020-09-01"),colour="blue") + geom_vline(xintercept = as.Date("2021-03-01"),colour="blue") + labs(title = "UNEMP to EMP")
...
COVIDyearUEtran <- UnemptoEmpFull %>% filter(year.x == 2020 | year.x == 2021 | year.x == 2022) %>% select(month, TUErate, year.x)
...
ggplot(UEtrandf,aes(x=month,y=TUErate,colour=year.x)) + geom_line() + geom_vline(xintercept = 4,colour="red",linetype = "dashed") + geom_vline(xintercept = 9,colour="blue",linetype = "dashed") + scale_y_continuous(labels=scales::percent_format(scale=100,suffix="%")) + labs(
  title = "Unemployment to employment",
  subtitle = "Monthly Transitions",
  y="",
  caption = "ABS National Accounts.
Source: ABS, e61.",
)  + theme_e61(base_family = "Quattrocento Sans", legend = "bottom") + scale_x_continuous(breaks = unique(UEtrandf$month))

save_e61(filename = 'C:/MN personal planning/Income_Support/plots/UE_Trans.svg')
...
COVIDyearEUtran <- EmptoUnempFull %>% filter(year.x == 2020 | year.x == 2021 | year.x == 2022) %>% select(month, TEUrate, year.x)
...
ggplot(EUtrandf,aes(x=month,y=TEUrate,colour=year.x)) + geom_line() + geom_vline(xintercept = 4,colour="red",linetype = "dashed") + geom_vline(xintercept = 9,colour="blue",linetype = "dashed") + scale_y_continuous(labels=scales::percent_format(scale=100,suffix="%")) + labs(
...
save_e61(filename = 'C:/MN personal planning/Income_Support/plots/EU_Trans.svg')
...
ggplot(EEtrandf,aes(x=month,y=TEErate,colour=year.x)) + geom_line() + geom_vline(xintercept = 4,colour="red",linetype = "dashed") + geom_vline(xintercept = 9,colour="blue",linetype = "dashed") + scale_y_continuous(labels=scales::percent_format(scale=100,suffix="%")) + labs(title = "Monthly transition from employed FT to employed PT")+ scale_x_continuous(breaks = unique(UEtrandf$month)) # %>% filter(year.x != 2022)
...
EU <- ggplot(EUtrandf,aes(x=Month,y=Transition,colour=Year)) + geom_line(size=0.9) + labs(
  title = "Monthly transition from employed to unemployed",
  subtitle = "Percent employed in prior month transitioning in month",
  y = "          ",
  x = "Month of year",
  caption = "ABS Labour Force Survey Gross Monthly Flows.
Source: ABS, e61.",
)  + theme_e61_clean() + scale_x_continuous(breaks = unique(EUtrandf$Month)) + scale_y_continuous(limits = c(0.000,0.02),labels=scales::percent_format(scale=100,suffix="%",sec.axis = dup_axis())) + scale_colour_e61(n=4) #+ geom_vline(xintercept = 4,colour="red",linetype = "dashed") + geom_vline(xintercept = 9,colour="blue",linetype = "dashed")
...
save_e61(filename = 'C:/MN personal planning/Income_Support/SAM/E2U.svg')

# Unemp - Emp

colnames(UEtrandf) <- c("Month","Transition","Year")

UE <- ggplot(UEtrandf,aes(x=Month,y=Transition,colour=Year)) + geom_line(size = 0.9)+ labs(
  title = "Monthly transition from unemployed to employed",
  subtitle = "Percent of unemployed in prior month transitioning in month",
  y = "          ",
  x = "Month of year",
  caption = "ABS Labour Force Survey Gross Monthly Flows.
Source: ABS, e61.",
) + theme_e61_clean() + scale_x_continuous_e61(breaks = unique(UEtrandf$Month)) + scale_y_continuous(limits=c(0.08,0.32),labels=scales::percent_format(scale=100,suffix="%")) + scale_colour_e61(n=4) #+ geom_vline(xintercept = 4,colour="red",linetype = "dashed") + geom_vline(xintercept = 9,colour="blue",linetype = "dashed")
...
save_e61(filename = 'C:/MN personal planning/Income_Support/SAM/U2E.svg')
