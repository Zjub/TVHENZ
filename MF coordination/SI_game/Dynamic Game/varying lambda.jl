...
@syms mu_G mu_B delta lambda pi_G_star pi_B_star y_G_star y_B_star beta_G_star beta_B_star
...
param_subs = Dict(
    ...
    beta_G_star => params_gov[18], beta_B_star => params_gov[19], lambda_G => params_gov[20]
)
...
for λ in lambdas
    ...
    push!(results, (λ, f_values[n], m_values[n], inflation, output))
end

# Convert results to DataFrame for easy plotting
using DataFrames
df = DataFrame(λ = [r[1] for r in results], f_100 = [r[2] for r in results], m_100 = [r[3] for r in results], 
               inflation_100 = [r[4] for r in results], output_100 = [r[5] for r in results])
