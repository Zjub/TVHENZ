...
calculate_EMTR <- function(wage = 25,hour_interest = 20, max_hours = 60, partnered = 0, partner_earnings = 0, over_60 = 0, living_alone = 0, Have_dep = 0, Main_carer_dep = 0, child_ages = "99",Rent=200,cite="") {
  ...
  calc_income_tax <- function(gross_income,work_income) {
    ...
    if (work_income == 0){ # The beneficiary tax offset just appears to say your tax is offset if you have no taxable income
      tax <- 0
    }
    else {
  ...
  calc_benefit_abated <- function(work_income,partner_earnings) {
    ...
    gross_benefit = calc_benefit_gross()[["taxable_benefit"]] # Set to this level if you want a supplement cliff-face
    #gross_benefit = calc_benefit_gross()[["gross_benefit"]] # Set to this if you want supplement abatement
    abate = 0
  ...
  calc_family_benefit <- function(child_age,gross_fam_income,work_income,abated_benefit){
    ...
    if (partnered == 0){
      ...
      net_fam_b = ifelse(max(partner_earnings,work_income)*(1+super_cont_rate) > fam_b_threshold,0,max(fam_b-0.2*(min(partner_earnings*(1+super_cont_rate),work_income*(1+super_cont_rate)+abated_benefit*26)),0))
    }
  ...
  calc_net_income <- function(wage_rate, hours) {
    ...
    fam_a_income = calc_family_benefit(child_age,gross_fam_income,work_income,abated_benefit)*26
  ...
  calc_effective_marginal_tax_rate <- function(wage_rate, hours) {
    ...
    if (hours >= 0) {
      ...
      new_income_tax <- calc_income_tax(new_gross_income,work_income + additional_income)
      
      new_family_payment <- calc_family_benefit(child_age,new_gross_income + partner_earnings,work_income + additional_income,new_benefit_payment)*26 
  ...
  average_tax_rate_calculator <- function(wage_rate,partner_earnings) {
    ...
    initial_income <- initial_gross_income - calc_income_tax(initial_gross_income,0) + calc_family_benefit(child_age,initial_gross_fam_income,0,calc_benefit_abated(0,partner_earnings)*26)*26 + calc_benefit_gross()[["supp_benefit"]]*26
    ...
    abatement_amount[hours + 1] <- calc_benefit_gross()[["taxable_benefit"]]*26 - calc_benefit_abated(wage_rate*hours*52,partner_earnings)*26
    }
