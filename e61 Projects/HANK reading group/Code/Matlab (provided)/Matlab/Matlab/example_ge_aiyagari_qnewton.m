...
param.aagrid = aagrid;
param.adelta = adelta;
param.aydelta = aydelta;
param.ygrid = ygrid;
param.yygrid = yygrid;
param.yytrans = yytrans;
param.ydist = ydist;
param.na = na;
param.ny = ny;
param.dagridf = dagridf;
param.dagridb = dagridb;
param.mindV = mindV;
param.u1inv = u1inv;
param.u = u;
param.u1 = u1;
param.rho = rho;
param.delta_hjb = delta_hjb;
param.maxiter_hjb = maxiter_hjb;
param.tol_hjb = tol_hjb;
    
%solve 
options_fsolve = optimoptions( 'fsolve','SpecifyObjectiveGradient',false,'Display','iter','FunctionTolerance',1e-6);
% [x,fval,exitflag,output] = fsolve(@(x)market_clearing(x,param),rguess,options_fsolve);
[x,fval,exitflag,output] = fzero(@(x)market_clearing(x,param),rguess);

[res,con,sav,gamarg] = market_clearing(rguess,param);
...
%%
function [res,con,sav,gamarg] = market_clearing(r,param)
     
    %extract    
    capshare = param.capshare;
    deprec = param.deprec;
    agrid = param.agrid;
    adelta = param.adelta;
    ygrid = param.ygrid;
    ...
    na = param.na;
    ny = param.ny;
    dagridf = param.dagridf;
    dagridb = param.dagridb;
    mindV = param.mindV;
    u1inv = param.u1inv;
    u = param.u;
    u1 = param.u1;
    ...
    ygrid = param.ygrid;
    yygrid = param.yygrid;
    yytrans = param.yytrans;
    rho = param.rho;
    delta_hjb = param.delta_hjb;
    aydelta = param.aydelta;
    maxiter_hjb = param.maxiter_hjb;
    tol_hjb = param.tol_hjb;
    
    %implied prices
    KLratio = ((r+deprec)/capshare)^(1/(capshare-1));
