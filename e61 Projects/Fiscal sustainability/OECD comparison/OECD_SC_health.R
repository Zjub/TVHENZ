# Topic: Initial play with SC methods for health spending
# Author: Matt Nolan
# Created: 14/7/2025
# Last edit: 14/7/2025
...
OECD_exp_GDP <- read_excel("table22-consolidated-cofog-expenditure-spent-by-approach.xlsx", 
                           sheet = "COFOGexp_%GDP", skip = 1)

setDT(OECD_exp_GDP)

# Collapse by country
OECD_health_dt <- OECD_exp_GDP[`COFOG Area` == "Health", lapply(.SD, sum,na.rm=TRUE), by = Country, .SDcols = patterns("^\\d{4}$")]
setDT(OECD_health_dt)

OECD_health_dt[,.(Country,`2022`)][order(`2022`)]


OECD_health_long_dt <- melt(OECD_health_dt,id.vars = "Country",variable.name = "year",value.name="value")[value > 0]
OECD_health_long_dt$year <-as.numeric(as.character(OECD_health_long_dt$year))
OECD_health_long_dt$group <- ifelse(OECD_health_long_dt$Country == "Australia", "Australia", "Other")

ggplot(OECD_health_long_dt[year > 1996], aes(x = year, y = value, colour = group, group = Country)) +
  geom_line() +
...
country_set <- OECD_health_long_dt[year == year_data & !Country %in% outliers]$Country
length(country_set)

OECD_forsynth <- OECD_health_long_dt[Country %in% country_set]
...
save_e61("SC_health_temp.png",res=2)
...
save_e61("SC_health_temp_age.png",res=2)
