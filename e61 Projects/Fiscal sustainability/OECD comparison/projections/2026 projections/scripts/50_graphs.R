...
p7 <- ggplot(top_anchor, aes(year, value * 100, colour = model_label)) +
  ...
  geom_vline(xintercept = official_forecast_end + 0.5, linetype = "dashed", colour = "grey45") +
  labs(title = "Top-down projections after the common official forecast anchor",
       subtitle = "Divergence after 2028-29 is entirely due to model dynamics and projected drivers",
