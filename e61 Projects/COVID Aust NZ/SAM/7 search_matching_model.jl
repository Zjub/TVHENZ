# Single agent model with probabilistic receipt.  Interesting, but not the question and the results don't make sense (the future values for both will be the same)
# search_matching_model_gridsearch_vacancy_leisure_bargaining_Bellman.jl
# Full model with:
# - Grid search calibration
# - Bellman fixed-point solution for W, U_N, U_R
...
exogenous_wage = true  # Set to false to use Nash bargaining - currently making exogenous as code is bugged.
...
function job_finding_rates(γ, μ, y, b_R)
    ...
    p_contact = rates.f
    # Fixed-point iteration for W, U_N, U_R
    ...
    f_N = s_N * p_contact
    f_R = s_R * p_contact
    return f_N, f_R, s_N, s_R, W, U_N, U_R, θ, w
end
