# Script to pull together the time series graphs and simple DiD - the placebo for the JFR using 2022 data
# Author: Matt Nolan
...
if (treatment_number == 1){
  prop_JSP_matched_dt <- read.csv("2022/JFR_by_group 1_placebo.csv")  # Main results JSP
  ...
  prop_JSP_matched_dt <- read.csv("2022/JFR_by_group 2_placebo.csv")  # Main results JSP
  setDT(prop_JSP_matched_dt)
...
ggplot(prop_JSP_matched_dt,aes(x=date,y=prop,colour=as.factor(nz))) + geom_line() + 
  ...
  plab(c("Announcement"),x=c(as.IDate("2022-04-01")),y=c(0.05),colour=c("black")) +
  plab(c("Australian","New Zealander"),x=c(as.IDate("2022-04-01"),as.IDate("2022-04-01")),y=c(0.13,0.145),colour=c(palette_e61(2)[1],palette_e61(2)[2])) +
...
save_e61("JFR_2022.pdf",res=2,pad_width = 1,auto_scale = FALSE)
