# Script to pull together the time series graphs and simple DiD - undertaken for our placebo group
# Author: Matt Nolan
...
if (treatment_number == 1){
  prop_JSP_matched_dt <- read.csv("JFR_noJSP_by_group 1 .csv")  # Main results noJSP
  ...
  prop_JSP_matched_dt <- read.csv("initial_RDD/JFR_noJSP_by_group 2 .csv")  # Main results noJSP
  setDT(prop_JSP_matched_dt)
...
ggplot(prop_JSP_matched_dt[date < as.IDate("2020-07-01")],aes(x=date,y=prop,colour=as.factor(nz))) + geom_line() + 
  ...
  plab(c("Announcement"),x=c(as.IDate("2020-01-23")),y=c(0.025),colour=c("black")) +
  #plab(c("Announcement","Payment Increase"),x=c(as.IDate("2020-02-01"),as.IDate("2020-05-01")),y=c(0.05,0.13),colour=c("black","black")) +
...
save_e61("JFR_level_diffnoJSP.png",res=2,pad_width = 1,auto_scale = FALSE)
...
save_e61("JFR_level_diffnoJSP.pdf",res=2,pad_width = 1,auto_scale = FALSE)
...
0.017058/pre_announce_avg[nz==0]
...
prop_JSP_matched_dt2 <- read.csv("JFR_noJSP_by_group 2 .csv")  # Main results JSP
...
save_e61("JFR_level_diff2noJSP.png",res=2,pad_width = 1,auto_scale = FALSE)
...
save_e61("JFR_level_diff2noJSP.pdf",res=2,pad_width = 1,auto_scale = FALSE)
